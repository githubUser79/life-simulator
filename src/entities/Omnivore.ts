...
import { OMNIVORE_BASE_RADIUS, CREATURE_MAX_SPEED, CREATURE_TURN_SPEED } from '../config';

export class Omnivore extends Creature {
  maxSpeed = CREATURE_MAX_SPEED;
  turnSpeed = CREATURE_TURN_SPEED;

  constructor(x: number, y: number, genome?: Genome) {
    super(x, y, OMNIVORE_BASE_RADIUS, genome);
  }
  ...
  reproduce(): Omnivore {
    ...
    const child = new Omnivore(this.position.x, this.position.y, childGenome);
