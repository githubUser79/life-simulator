...
import { PREY_BASE_RADIUS, CREATURE_MAX_SPEED, CREATURE_TURN_SPEED } from '../config';
...
export class Prey extends Creature {
  ...
  constructor(x: number, y: number, genome?: Genome) {
    super(x, y, PREY_BASE_RADIUS, genome);
    // Prey start with slight toxicity and defense (evolution will tune it)
    if (!genome) { // only for fresh spawns, not children
      this.toxicity = 0.1 + Math.random() * 0.15;
      this.defense = 0.05 + Math.random() * 0.1;
    }
  }
  ...
  reproduce(): Prey {
    ...
    const child = new Prey(this.position.x, this.position.y, childGenome);
